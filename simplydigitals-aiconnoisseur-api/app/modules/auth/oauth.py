    ...
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            raise HTTPException(
                ...
                detail=t("auth.google_unavailable"),
            ) from exc
    if resp.status_code != 200:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=t("auth.google_token_invalid"),
        )
    data = resp.json()
    if settings.GOOGLE_CLIENT_ID and data.get("aud") != settings.GOOGLE_CLIENT_ID:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=t("auth.google_audience_mismatch"),
        )
    email = data.get("email")
    if not email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=t("auth.google_no_email"),
        )
    return OAuthProfile(
...
async def verify_facebook_token(
    ...
    if not settings.FACEBOOK_APP_ID or not settings.FACEBOOK_APP_SECRET:
        raise HTTPException(
            ...
            detail=t("auth.facebook_not_configured"),
        )
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=t("auth.facebook_unavailable"),
            ) from exc
    if debug_resp.status_code != 200:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=t("auth.facebook_token_invalid"),
        )
    ...
    if debug_data.get("app_id") != settings.FACEBOOK_APP_ID:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=t("auth.facebook_wrong_app"),
        )
    proof = hmac.new(
    ...
    if not email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=t("auth.facebook_no_email"),
        )
